...
class Refresh():
    ...
    name = "Рефреш"
    price = "цена рефреш"
    discr = "финальный курс года"

class Fresh():

    name = "Рефреш"
    price = "цена фреш"
    discr = "подготовка за 3 месяца"

class Extra():

    name = "Экстра"
    price = "Цена экстра"
    discr = "полугодовой курс"
...
def callback_inline(call):
    if call.data == 'mainmenu':
        ...
        bot.send_message(call.message.chat.id,
                         'Здесь информация про курс {}, сразу подскажу стоимость {}. {}'.format(osnova.name, osnova.price, osnova.discr),
        ...
        menu_fresh.add(back)
        bot.send_message(call.message.chat.id,
                         'Здесь информация про курс {}, сразу подскажу стоимость {}. {}'.format(fresh.name, fresh.price, fresh.discr),
        ...
        menu_extra.add(back)
        bot.send_message(call.message.chat.id,
                         'Здесь информация про курс {}, сразу подскажу стоимость {}. {}'.format(extra.name, extra.price, extra.discr),
        ...
        bot.send_message(call.message.chat.id,
                         'Здесь информация про курс {}, сразу подскажу стоимость {}. {}'.format(refresh.name, refresh.price, refresh.discr),
        ...
        menu_spec.add(back)
        bot.send_message(call.message.chat.id,
                         'Здесь информация про курс {}, сразу подскажу стоимость {}. {}'.format(spec.name, spec.price, spec.discr),
